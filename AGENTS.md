...
- `scripts/meta-collect/` — Paimon 元数据周期批采集（crontab + Flink SQL Batch，无状态简化版）。
  ...
  经 HDFS du/ls 采集，同走 Kafka→ODS（`rdw_ods_paimon_meta_physical`）；Kafka topic 生产速率
  由 `bin/collect_kafka_offsets.sh` 采分区末端位移、分析层视图差分
  （`rdw_ods_kafka_topic_offsets` / `v_kafka_topic_write_rate`）。详见其 README.md。
- `docs/` — 需求、设计、验证文档（中文）。
