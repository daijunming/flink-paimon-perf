...
> - 流式写入（高频 I/U/D）
> - 独立合并（crontab 每 5 分钟提交一次的 BATCH 模式合并任务 `paimon-compact`，跑完即退出）
...
| **场景 2：单合并性能** | L0 堆积、COMPACT 提交节奏、文件替换量、资源占用 | `wide_table`<br>`cluster`<br>（`paimon-compact` 批任务不上报作业侧指标） | `metrics_resource_compaction`<br>`health_flags`<br>⚠️ `metrics_compaction_job` 在批形态下无数据 |
...
## ⚙️ 场景 2：Compaction 性能测试（crontab 周期批式合并）

### 测试目的
评估**独立合并任务**在高频 I/U/D 场景下的合并能力、资源开销和文件管理效率。

**现场真实形态（2026-08-13 调整）**：合并由 **crontab 每 5 分钟提交一次的 BATCH 模式任务**完成——`flink run-application -t yarn-application -Dexecution.runtime-mode=BATCH ... paimon-flink-action-1.1.1.jar compact --compact_strategy minor`，作业名 `paimon-compact`，跑完即退出。**不是**流式常驻 `compaction_job`；BATCH 只规定任务生命周期，文件选择由 `minor` 决定。

⚠️ **由此带来的观测口径变化**：批任务生命周期（几十秒）短于指标上报周期（3 分钟一批），`compactionThreadBusy`、`avgCompactionTime` 等作业侧指标基本不会出现在 `RDW_ODS_FLINK_METRICS`；且 `paimon-compact` 不在 `01_metrics_view.sql` 的 job_name 白名单内。因此 **`metrics_compaction_job` 视图与 `health_flags.compaction_flag` 在当前形态下无数据**——查不到不代表合并未运行。作业侧运行与否只能看 **cron 日志**（是否每 5 分钟提交）与 **YARN 应用历史**（`paimon-compact` 应用是否陆续 SUCCEEDED）；**compact 效果分析以表侧元数据为准**（COMPACT 提交节奏、文件替换量差分、L0 曲线、有效体积 vs 磁盘总占用），口径见 `docs/写入与合并性能分析.md`。

### 前置验证（运行测试前确认）
- [ ] 确认 crontab 批式合并正常轮转：cron 日志有每 5 分钟一次的提交记录，YARN 应用历史中 `paimon-compact` 应用陆续 SUCCEEDED
- [ ] 确认 metadata-collector 正常采集文件数据（`paimon.level.file.count.*` 指标存在）——批形态下合并效果主要看这条链路
- [ ] 确认 resource-collector 正常采集集群资源（`yarn.*`、`hdfs.*` 指标存在）
- [ ] （仅切回流式常驻 `compaction_job` 形态时）确认 Paimon 桥接指标已上报（`compactionThreadBusy`、`avgCompactionTime`）
...
| 指标名称 | metric_name（LIKE 匹配） | 观测目的 | 健康标准 |
|---------|------------------------|---------|---------|
| **Compaction 繁忙度（%）** | `%compactionThreadBusy` | 合并线程是否满负荷<br>⚠️ **仅切回流式常驻 `compaction_job` 形态时有效**，当前 crontab 批形态下无数据 | **良好**：< 70%<br>**注意**：70-90%<br>**告警**：≥ 90%（流式形态下触发 `health_flags.compaction_flag = 'COMPACTION_SATURATED'`，需加资源；批形态下该标志无数据） |
| **L0 文件堆积** | `paimon.level.file.count.L0` | 最直接的合并滞后信号，**批形态下判断合并是否跟上的首要指标** | **良好**：稳定或缓慢增长<br>**告警**：持续快速增长（触发 `health_flags.l0_flag = 'L0_PILEUP'`） |
| **Compaction 平均耗时（ms）** | `%avgCompactionTime` | 单次合并的时间代价<br>⚠️ **仅切回流式常驻 `compaction_job` 形态时有效**，当前 crontab 批形态下无数据 | **良好**：< 30 秒<br>**注意**：30-60 秒<br>**告警**：> 60 秒 |
| **Compaction 提交占比** | `paimon.last.commit.kind`<br>（编码值：COMPACT=1.0 / APPEND=0.0 / 其他=0.5） | 合并触发频率（批形态下体现为每 5 分钟一轮的 COMPACT 提交节奏） | 根据 U/D 比例（约 50%）合理即可，无固定标准 |
...
### 数据来源
- `paimon-compact`：crontab 批式合并任务（BATCH 模式，跑完即退出；作业侧指标基本不上报，见测试目的节的口径说明）
- `wide_table`：Paimon 元数据采集器（轮询 $files/$snapshots）——**批形态下合并效果的主观测来源**
- `cluster`：YARN/HDFS REST API 采集器

### 分析视图

⚠️ **先看这里**：当前 crontab 批形态下，`metrics_compaction_job` 与 `health_flags.compaction_flag` 查不到数据（原因见测试目的节）。compact 效果的完整分析口径（COMPACT 提交节奏、文件替换量差分、L0 曲线、有效体积 vs 磁盘总占用）以 `docs/写入与合并性能分析.md` 为准；下面视图里仍然有效的是表侧元数据与资源部分。

```sql
-- Compaction 作业性能（繁忙度 / 平均耗时）
-- ⚠️ 仅流式常驻 compaction_job 形态下有数据；当前批形态下结果为空，属预期
SELECT * FROM RDW_DATA.metrics_compaction_job
WHERE time_bucket_minute BETWEEN '<起>' AND '<止>'
ORDER BY time_bucket_minute;

-- 文件堆积与资源占用（文件数 / L0-L5 / YARN / HDFS）——批形态下的主视图
...
-- 健康软标志（L0 堆积 / 反压仍有效；compaction_flag 在批形态下无数据）
...
**前提条件**：被读表 `changelog-producer=lookup`，且独立 Compact Action 已经现场核实能产出完整 RowKind（当前配置已对齐，正确性仍以 I/U/U/D 冒烟结果为准）。
...
**启动顺序**（避免相互干扰）：
1. **确认合并任务在轮转**：合并由 crontab 每 5 分钟提交的 BATCH 任务 `paimon-compact` 自动完成（跑完即退出），无需手工启动；检查 cron 日志与 YARN 应用历史确认周期提交正常
...
| **合并侧影响** | L0 堆积是否加剧、COMPACT 提交节奏是否正常（繁忙度/耗时仅流式形态可得） | 场景 2 的核心指标 |
...
-- ⚠️ 批形态下 metrics_compaction_job 无数据，下面与它 INNER JOIN 会整体查空；
--    需把该 JOIN 改为 LEFT JOIN（c.* 两列为 NULL）或直接去掉，
--    合并侧改用表侧元数据口径（COMPACT 提交节奏 / 文件替换量，见 docs/写入与合并性能分析.md）
SELECT
    w.time_bucket_minute,
    w.throughput_rps,                 -- 写入吞吐（场景 1）
    h.max_checkpoint_start_delay_ms,  -- 写入反压（场景 1）
    c.compaction_thread_busy_max,     -- 合并繁忙度（场景 2，仅流式形态有值）
    c.avg_compaction_time_ms,         -- 合并耗时（场景 2，仅流式形态有值）
...
| `paimon-compact` | 合并任务（crontab 每 5 分钟提交的 BATCH 模式 Paimon Action compact，跑完即退出） | 作业侧指标基本不上报（生命周期 < 3 分钟上报周期，且不在白名单）；繁忙度/耗时仅切回流式常驻 `compaction_job` 形态时可得 |
...
- ⚠️ 延迟探针代码已移除（2026-08-06 清理），`ingest.e2e_latency_ms` 从不产出，可忽略
