...
**现场真实形态（2026-08-06 核对）**：合并由 **crontab 每 5 分钟提交一次的 BATCH 模式任务**完成——`flink run-application -t yarn-application -Dexecution.runtime-mode=BATCH ... paimon-flink-action-1.1.1.jar compact --compact_strategy full`，作业名 `paimon-compact`，几十秒跑完即退出。**不是** `scripts/sql/06_compaction_job.sh` 描述的流式常驻 `compaction_job`（该形态已不采用）。
